...
public class EndlessTrackGenerator : MonoBehaviour
{
    ...
    private int FindTrackIndexWithPlayer()
    {
        for (int i = 0; i < currentSegments.Count; i++)
        {
            ...
            if (player.transform.position.z >= track.StartZ.position.z + minDistanceToConsiderInsideTrack &&
                player.transform.position.z <= track.EndZ.position.z)
            {
                return i;
            }
        }
    ...
    private TrackSegment SpawnTrackSegment(TrackSegment track, TrackSegment previousTrack)
    {
        ...
        {
            trackInstance.transform.position = previousTrack.EndZ.position + (trackInstance.transform.position - trackInstance.StartZ.position);
        }
        else
        {
